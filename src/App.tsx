...
const BUTTON_CLASSES = {
  whatsapp: 'bg-gradient-to-r from-green-400 via-emerald-500 via-green-500 to-teal-400 text-white py-3 sm:py-4 px-2 sm:px-4 relative overflow-hidden group shadow-lg',
  cart: 'absolute top-2 sm:top-3 right-3 sm:right-6 lg:right-[8vw] hover:scale-110 active:scale-95 transition-all duration-300 ease-out drop-shadow-lg border-2 border-white/80 rounded-xl p-2 sm:p-2.5 bg-white/10 backdrop-blur-sm group/cart cursor-pointer z-50'
};
...
function App() {
  ...
  // =================== MEMOIZED CALLBACKS ===================
  const memoizedAddItem = useCallback(addItem, [addItem]);
  ...
  const renderWhatsAppIcon = () => (
    <div className='relative flex-shrink-0'>
      <div className='relative animate-bounce'>
        <svg
          viewBox='0 0 24 24'
          className='w-7 h-7 sm:w-9 sm:h-9 md:w-10 md:h-10 transform group-hover:scale-110 group-hover:rotate-12 transition-all duration-500 ease-out filter drop-shadow-lg'
          fill='currentColor'
          aria-hidden="true"
        >
  ...
  const renderWhatsAppButton = () => (
    <div className={BUTTON_CLASSES.whatsapp}>
      ...
      <a
        ...
        className='flex items-center justify-center space-x-2 sm:space-x-4 font-bold text-lg sm:text-xl md:text-2xl hover:text-green-100 transition-all w-full text-center px-1 sm:px-2 group relative z-10'
      ...
      >
        ...
        <div className='flex flex-col items-center justify-center gap-0.5 sm:gap-1 min-w-0 flex-1 text-center'>
          <div className='flex flex-col sm:flex-row items-center justify-center gap-1 sm:gap-3 w-full'>
            <span className='text-lg sm:text-xl md:text-2xl lg:text-3xl font-extrabold tracking-wide group-hover:tracking-wider transition-all duration-300 whitespace-nowrap transform group-hover:scale-105 drop-shadow-md text-center'>
              Jetzt bestellen! 🍕
            </span>
            <span className='text-base sm:text-lg md:text-xl lg:text-2xl font-bold tracking-wide group-hover:tracking-wider transition-all duration-300 whitespace-nowrap transform group-hover:scale-105 bg-white/20 px-2 py-0.5 rounded-full backdrop-blur-sm text-center mx-auto sm:mx-0'>
              📞 {CONTACT_INFO.PHONE}
            </span>
          </div>
          
          <div className='text-xs sm:text-sm opacity-90 animate-pulse font-medium text-center'>
  ...
  const renderCartButton = () => (
    ...
    >
      <ShoppingCart className='w-6 h-6 sm:w-7 sm:h-7 group-hover/cart:animate-bounce' aria-hidden="true" />
      ...
      {totalItemsCount > 0 ? (
        <span 
          className='absolute -bottom-1 -right-1 px-1.5 py-0.5 rounded-full bg-gradient-to-r from-yellow-400 to-orange-400 text-black text-xs font-bold min-w-[1.5rem] h-6 flex justify-center items-center animate-bounce border-2 border-white shadow-lg'
          aria-hidden="true"
        >
          {totalItemsCount}
        </span>
        ...
        <div 
          className='absolute -top-0.5 -right-0.5 w-3 h-3 bg-red-400 rounded-full animate-ping opacity-75'
          aria-hidden="true"
        ></div>
      )}
    </button>
          ...
          <div className='grid grid-cols-1 lg:grid-cols-3 gap-6 lg:gap-8'>
            <div className='lg:col-span-2 space-y-8'>
              ...
              {renderMenuSection(
                MENU_SECTIONS.PIDE,
                'Pide',
                'Alle Pides werden mit Käse zubereitet',
                pides
              )}
              ...
              {renderMenuSection(
                ...
                'Salate',
                'Alle Salate werden mit einem Dressing Ihrer Wahl zubereitet (z.B. Joghurt, Balsamico- oder Essig-Öl-Dressing)',
                salads
              )}
            ...
            <div
              id='cart'
              data-cart-section="true"
              className='lg:sticky lg:max-h-[calc(100dvh-150px)] min-w-[360px] md:min-w-[480px] overflow-y-auto overflow-x-hidden lg:top-[8.5rem] flex flex-col justify-start items-center scroll-mt-32 border-2 border-transparent rounded-lg'
            >
              <OrderForm
                orderItems={items}
